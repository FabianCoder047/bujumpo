...
// Bateaux par mois
$stmt = $db->prepare("SELECT DATE_FORMAT(b.date_entree, '%Y-%m') m, COUNT(DISTINCT UPPER(REPLACE(REPLACE(TRIM(b.immatriculation),' ',''),'-',''))) c FROM bateaux b WHERE b.date_entree BETWEEN ? AND ? AND (b.date_sortie IS NULL OR b.date_entree < b.date_sortie) GROUP BY m");
$stmt->execute([$start_ts, $end_ts]);
$bateaux_entree_map = [];
foreach ($stmt->fetchAll() as $r) { $bateaux_entree_map[$r['m']] = (int)$r['c']; }

$stmt = $db->prepare("SELECT DATE_FORMAT(b.date_sortie, '%Y-%m') m, COUNT(DISTINCT UPPER(REPLACE(REPLACE(TRIM(b.immatriculation),' ',''),'-',''))) c FROM bateaux b WHERE b.date_sortie BETWEEN ? AND ? GROUP BY m");
$stmt->execute([$start_ts, $end_ts]);
$bateaux_sortie_map = [];
...
$stmt = $db->prepare("SELECT COUNT(DISTINCT UPPER(REPLACE(REPLACE(TRIM(b.immatriculation),' ',''),'-',''))) t FROM bateaux b WHERE b.date_entree BETWEEN ? AND ? AND (b.date_sortie IS NULL OR b.date_entree < b.date_sortie)");
$stmt->execute([$start_ts, $end_ts]);
$total_bateaux_entree_periode = (int)$stmt->fetch()['t'];
$stmt = $db->prepare("SELECT COUNT(DISTINCT UPPER(REPLACE(REPLACE(TRIM(b.immatriculation),' ',''),'-',''))) t FROM bateaux b WHERE b.date_sortie BETWEEN ? AND ?");
$stmt->execute([$start_ts, $end_ts]);
...
$month_type_map = [];
// Camions - Entrée (mois) — par date d'entrée camion
...
// Camions - Sortie (mois) — par date de sortie camion
$stmt = $db->prepare("SELECT tm.nom t, SUM(mc.poids) s
    FROM marchandises_camions mc
    JOIN types_marchandises tm ON tm.id = mc.type_marchandise_id
    JOIN camions c ON c.id = mc.camion_id
